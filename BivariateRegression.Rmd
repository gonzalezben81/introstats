...
print(mtcars$mpg)
# knitr::kable(mtcars$mpg, caption = "Actual y values")

print('Manually Calculated Residuals')
table<- as.data.frame(mtcars$mpg-mpg_prediction$fitted.values)
print(table)
# knitr::kable(table, caption = "Residuals of Prediction")
# d1 <- table[1:16,]
# d2 <- table[17:32,]
# knitr::kable(
#   list(d1, d2),
...
mpg_residual_data_frame <- data.frame(
  
  actual_y_values = mtcars$mpg,
  predicted_y_values = mpg_prediction$fitted.values,
  residual_values = residual_mpg_values
  
)

reactable::reactable(mpg_residual_data_frame,columns = list(
  .rownames = colDef(name = c("Vehicle Make"), sortable = TRUE),
  actual_y_values = colDef(name = "Actual Y Values"),
  predicted_y_values = colDef(name = "Predicted Y Values"),
  residual_values = colDef(name = "Residual")
...
```{r}
mpg_hp_model <- lm(mpg ~ hp, mtcars)
mpg_hp_model$coefficients
```

#### Print the fitted values of the model
```{r}
mpg_hp_model <- lm(mpg ~ hp, mtcars)
mpg_hp_model$fitted.values
```
